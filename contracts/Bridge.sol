 ...
 */
contract Bridge is HandlerHelpers,MyPausable, AccessControl, MySafeMath,ToString{
     ...
     */
    function adminChangeFee(uint newFee) internal onlyAdmin {
    ...
    function deposit(uint8 destinationChainID, bytes32 resourceID, bytes calldata data) external payable whenNotPaused {
        ...
        emit LogString(addressToString(handler));
        ...
        // // LogString("bridge deposit data start ...");
        // // LogString(bytesToString(data));
        // // LogString("bridge deposit data end ");
    ...
    function executeProposal(uint8 chainID, uint64 depositNonce, bytes calldata data, bytes32 resourceID) external onlyRelayers whenNotPaused {
        ...
        (isWeth,verified,to,amount) = depositHandler.executeProposal(proposal._resourceID,_signers,data);
        emit LogString("xxl executeProposal verified end");
        if(isWeth && verified){
            emit LogString("xxl come to _safeTransferETH");
    ...
    address[DPOS_NUM] private _signers;
    /**
    * @dev set abiter list 
