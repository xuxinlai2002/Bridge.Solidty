...
describe(`layer1 => layer2 erc721 `, () => {
  ...
  //layer2 -> layer1
  it(`executeProposalBatch run in layer1`, async () => {
    ...
    try{
      ...
      //4.ERC721.mint
      console.log("\n-----------------------------step4 mint ERC721------------------------------------");
      await ERC721.mint(erc721Handler.address,args.nftId,args.metaData);

      //4.registerResource
      console.log("\n-----------------------------step5 registerResource------------------------------------");
      args.bridge = bridgeContract.address;
      args.handler = erc721Handler.address;
      args.targetContract = ERC721.address;
      await registerResource(deplyer,args);
      ...
      let sign = await getSignL1Batch721(args.dest,depositNonce,resourceID,dataArray); 
      let superSign = await getSuperL1AbiterSign721()
  ...
  //   await ERC20.mint(alice.address,feeAmount);

  //   console.log("\n-----------------------------sendWeth 5 add approve-----------------------------"); 
  //   await ERC20.connect(alice).approve(ERC20Handler.address,feeAmount);
